...
def plot_summary(df: pd.DataFrame, rank_df: pd.DataFrame) -> None:
    ...
    ax.bar(np.arange(len(order)), vals, yerr=sems, color="#0072B2", alpha=0.85, capsize=2)
    ax.set_xticks(np.arange(len(order)))
    ax.set_xticklabels(["All", "SOTA", "Train", "Arch", "Data"], rotation=0)
    ax.set_ylim(0, 1.05)
    ...
    fig.tight_layout()
    for ext in ["pdf", "png"]:
        fig.savefig(fig_dir / f"mixed_distance_metric_robustness.{ext}", dpi=200)
    plt.close(fig)
...
def main() -> None:
    ...
    if not df.empty and not rank_df.empty:
        plot_summary(df, rank_df)
