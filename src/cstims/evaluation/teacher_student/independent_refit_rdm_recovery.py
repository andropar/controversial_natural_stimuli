...
This is a diagnostic variant of ``compute_teacher_student_independent_refit.py``.
Readouts are still fitted from candidate raw features to teacher response targets
on an independent 1k natural-image pool, but held-out recovery is scored using
the RSA observation space used by the stimulus-selection analyses:
...
def select_targetwise_alpha_indices(
    alpha_ops: IndependentRidgeOps | dict[float, tuple[np.ndarray, dict[str, np.ndarray]]],
    y_train: np.ndarray,
    y_val: np.ndarray,
    ...
    projected_y = (
        alpha_ops.project_targets(y_train)
        if isinstance(alpha_ops, IndependentRidgeOps)
        else None
    )
    if isinstance(alpha_ops, IndependentRidgeOps):
        ...
        for alpha in alpha_values:
            coeff = alpha_ops.coefficients_from_projected_targets(alpha, projected_y)
            coefficient_cache[float(alpha)] = coeff
            coeffs_by_alpha.append(coeff)
        n_targets = y_train.shape[1]
        pred_val_all = alpha_ops.validation_prediction_from_coefficients(
    ...
    *,
    ...
    if eval_refit_mode == "eval_augmented_loo":
        if y_base_fit_clean is None:
            raise RuntimeError("Missing base fit targets for eval_augmented_loo")
        if (
...
def run_track_rdm_recovery(
    *,
    ...
    for teacher_idx, teacher in enumerate(model_names):
        if teacher_indices is not None and teacher_idx not in teacher_indices:
            continue
        cache_path = None
        if teacher_cache_dir is not None:
            cache_path = (
                teacher_cache_dir
                / f"{safe_name(track['name'])}__{teacher_idx:03d}__{safe_name(teacher)}.csv"
            )
        ...
        for noise_mult in noise_mults:
            ...
            for noise_sample_idx in range(n_noise_samples):
                ...
                for candidate_idx, candidate in enumerate(model_names):
                    ...
                    for key in eval_keys_for_ops(candidate_ops[candidate], alpha_values):
