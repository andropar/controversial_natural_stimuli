...
- `01_compute_crsa.py`: computes classical/fixed RSA (`crsa`). It extracts model
  ...
  writes `../../rsa_scores/<subject>/crsa_scores.csv`.
...
- Brain caches from `../brain_data_preparation/`, accessed through the stage
  path helpers as `inputs/` or copied heavy cache locations.
- Cstim and Vicco image payloads from the external HDF5/image root documented in
  `../../../external_data/README.md`.
- Model metadata from `../../../00_stimulus_selection/inputs/resources/`.
- Fitted encoding models from `../../inputs/encoding_models/` for
  `wrsa_transfer`.

Outputs:

- `../../rsa_scores/sub-*/crsa_scores.csv`
- `../../rsa_scores/sub-*/wrsa_transfer_scores.csv`
- `../../rsa_scores/sub-*/cross_set_wrsa_scores.csv`
- `../../rsa_scores/rsa_large_benchmark_scores.csv`
- `../../rsa_scores/benchmark_upper_tail_deltas*.csv`
