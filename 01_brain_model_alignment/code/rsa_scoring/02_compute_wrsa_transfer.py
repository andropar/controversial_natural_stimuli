#!/usr/bin/env python3
"""
Compute weighted RSA (wRSA) using transfer from DeepVision-trained encoding models.

Usage:
    python 03_compute_wrsa_transfer.py                  # All subjects
    python 03_compute_wrsa_transfer.py --subject sub-05  # Single subject

Model features are extracted ONCE (stimuli are identical across subjects).
Encoding models and brain RDMs change per subject.

Outputs per subject (in data/{subject}/):
    wrsa_transfer_scores.csv - wRSA scores using transferred encodings
"""
...
from utils import (
    ...
    get_encoding_folder,
    load_encoding_model,  # canonical version (was inline)
    predict_voxel_responses,  # canonical version (was inline)
    parse_subject_arg,
...
def load_subject_brain_data(subject: str) -> dict:
    ...
    for group in available_groups:
        ...
        # Vicco stim_idx is 1-based (from filenames), controversial groups are 0-based
...
def main():
    parser = argparse.ArgumentParser(description="Compute wRSA-transfer scores")
    ...
    for subject in subject_data:
        out_path = (RSA_DATA_DIR / subject / "wrsa_transfer_scores.csv")
        if out_path.exists():
            out_path.unlink()
    ...
    for model_set, models in MODEL_SETS.items():
        ...
        for model in tqdm(models, desc="Models"):
            ...
            # Inner loop: subjects (encoding + brain RDMs change)
            for subject, sdata in subject_data.items():
                ...
                try:
                    ...
                    # Predicted RDM for controversial stimuli
                    pred_cstim = predict_voxel_responses(subj_model_features_cstim, encoding)
                    pred_rdm_cstim = compute_rdm_correlation(pred_cstim[:, encoding_hlvis])

                    # Brain RDM
                    brain_rdm_cstim = compute_rdm_correlation(betas[:, cstim_brain_idx].T)

                    wrsa_cstim = compute_rsa_score(pred_rdm_cstim, brain_rdm_cstim, method="spearman")
                    ...
                    for boot_idx, vicco_subset_idx in enumerate(sdata["vicco_bootstrap"]):
                        ...
                        pred_vicco = predict_voxel_responses(model_features_vicco, encoding)
                        pred_rdm_vicco = compute_rdm_correlation(pred_vicco[:, encoding_hlvis])

                        wrsa_vicco = compute_rsa_score(pred_rdm_vicco, brain_rdm_vicco, method="spearman")
                    ...
                    print(f"  Error processing {model} for {subject}: {e}")
                    continue
