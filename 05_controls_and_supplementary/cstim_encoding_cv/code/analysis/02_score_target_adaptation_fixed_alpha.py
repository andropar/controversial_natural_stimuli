#!/usr/bin/env python3
"""Compute fixed-alpha SRP5920 target-adaptation scores.

This is the canonical fixed-alpha target-adaptation protocol:
...
from cstims.target_adaptation import (
    ...
    load_original_reference as load_original_reference_from_csv,
    parse_weights,
    rank_vector,
    rdm_corr,
    rdm_upper_vec,
    ...
    sanitize_layer_name,
    zscore_features_from_deepvision,
    zscore_targets_by_voxel,
)
...
def load_best_shared_selections() -> pd.DataFrame:
    return load_best_shared_selections_from_csv(SELECTION_CSV)


def load_original_reference() -> pd.DataFrame:
    return load_original_reference_from_csv(SELECTION_CSV, baseline_set=BASELINE_SET)
...
def load_deepvision_responses(subject: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    betas_npy = cache_dir / "voxel_betas.npy"
    metadata_csv = cache_dir / "voxel_metadata.csv"
    if not betas_npy.exists() or not metadata_csv.exists():
        raise FileNotFoundError(
            f"Missing cached DeepVision unique responses for {subject}: {cache_dir}"
        )
    responses = np.load(betas_npy, mmap_mode="r")
    metadata = pd.read_csv(metadata_csv)
    hlvis = metadata["hlvis"].to_numpy(dtype=bool)
    return np.asarray(responses[hlvis].T, dtype=np.float32), hlvis


def load_cstim_subject_data(subject: str) -> dict:
    cache = load_cstim_brain_cache(subject, cache_root=BRAIN_CACHE_ROOT)
    group_brain_idx = {}
    group_file_idx = {}
    for group in [*CSTIM_SETS, BASELINE_SET]:
        group_brain_idx[group] = cache.brain_indices(group)
        group_file_idx[group] = cache.feature_indices(group)
...
def target_sets_for_model(model: str, *, score_membership_only: bool) -> list[str]:
    ...
    return [model_set for model_set in CSTIM_SETS if model in set(constants.MODEL_SETS[model_set])]
...
def completed_resume_weight_rows(
    existing: pd.DataFrame,
    selections: pd.DataFrame,
    *,
    weights: list[float],
    score_membership_only: bool,
) -> tuple[dict[tuple[str, str], set[float]], pd.DataFrame]:
    if existing.empty:
    ...
    complete: dict[tuple[str, str], set[float]] = {}
    kept_blocks = []
    for row in selections.itertuples(index=False):
        ...
        for weight in weights:
            weight_block = block[
                np.isclose(block["target_weight"].astype(float), float(weight))
            ].copy()
            has_current_features = (
                len(weight_block) == expected
                and weight_block["feature_dim_analysis"].eq(SRP_TARGET_DIM).all()
                and weight_block["feature_protocol"].eq(FEATURE_PROTOCOL).all()
            )
            if not has_current_features:
                continue
            complete.setdefault(key, set()).add(float(weight))
            kept_blocks.append(weight_block)

    if kept_blocks:
        kept = pd.concat(kept_blocks, ignore_index=True)
    else:
        kept = pd.DataFrame(columns=existing.columns)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep complete subject/model/weight rows already present in the score CSV "
            "and compute only missing weights."
        ),
    )
    ...
    if ready.empty:
        raise RuntimeError(
            f"No SRP5920 feature-ready selections. Run 01_cache_selected_layer_srp5920_features.py. "
            f"See {AUDIT_CSV}"
        )
    ...
    rows = []
    completed_weights: dict[tuple[str, str], set[float]] = {}
    n_resume_rows = 0
    if args.resume and SCORE_CSV.exists():
        existing_scores = pd.read_csv(SCORE_CSV)
        completed_weights, kept_scores = completed_resume_weight_rows(
            existing_scores,
            ready,
            weights=weights,
            score_membership_only=score_membership_only,
        )
        rows = kept_scores.to_dict("records")
        n_resume_rows = len(rows)
        n_complete_selection_weights = sum(len(v) for v in completed_weights.values())
        n_complete_selections = sum(
            1 for v in completed_weights.values() if len(v) == len(weights)
        )
        print(
            f"[resume] keeping {n_resume_rows} rows from "
            f"{n_complete_selection_weights} complete selection-weight blocks; "
            f"{n_complete_selections} selections already have all requested weights",
            flush=True,
        )
    ...
    for idx, row in enumerate(ready.itertuples(index=False), start=1):
        ...
        done_weights = completed_weights.get((sel.subject, sel.model), set())
        missing_weights = [
            weight
            for weight in weights
            if not any(np.isclose(float(weight), done) for done in done_weights)
        ]
        ...
        print(
            ...
            f"weights={','.join(f'{w:g}' for w in missing_weights)}",
            flush=True,
        )
        rows.extend(
            compute_one_selection(
                sel,
                target_weights=missing_weights,
    ...
    meta = {
        ...
        "n_resume_rows": int(n_resume_rows),
        "n_resume_complete_selection_weights": int(
            sum(len(v) for v in completed_weights.values())
        ),
        "n_resume_complete_selections": int(
            sum(1 for v in completed_weights.values() if len(v) == len(weights))
        ),
