...
EXTREME_WEIGHT_CSV = RESULTS_DIR / "target_adaptation_fast_extreme_weight_scores.csv"
...
def load_scores(*, include_extreme: bool) -> pd.DataFrame:
    ...
    if include_extreme and EXTREME_WEIGHT_CSV.exists():
        extreme = pd.read_csv(EXTREME_WEIGHT_CSV)
        df = pd.concat([df, extreme], ignore_index=True, sort=False)
