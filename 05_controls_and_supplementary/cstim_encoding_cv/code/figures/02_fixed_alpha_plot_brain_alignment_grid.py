#!/usr/bin/env python3
"""Brain-alignment-style target-adaptation comparison with all subject points."""
...
from _paths import FIGURES_DIR, RESULTS_DIR, SHARE_ROOT
...
ROW_SPECS = [
    ...
    ("cstim_adapted", "CSTIM target-adapted", "refit_cstim_loso", "refit_vicco_heldout"),
    ("symmetric_loso", "Separate target LOSO", "refit_cstim_loso", "refit_vicco_loso"),
]
...
def load_scores(weight: float) -> pd.DataFrame:
    ...
    df = df[np.isclose(df["target_weight"], weight)].copy()
    ...
    heldout = df[df["eval_target"].eq("vicco_heldout")][
        keys + ["mrsa_loso", "original_best_shared_mrsa"]
    ].rename(
        columns={
            "mrsa_loso": "refit_vicco_heldout",
            "original_best_shared_mrsa": "original_vicco",
        }
    )
    vicco_keys = ["subject", "model", "display_name", "selected_layer"]
    vicco_loso = df[df["eval_target"].eq("vicco_loso")][
        vicco_keys + ["mrsa_loso"]
    ].rename(columns={"mrsa_loso": "refit_vicco_loso"})
    ...
    if wide.empty:
        raise RuntimeError(f"No matched target-adaptation rows for target_weight={weight:g}")
    return wide
...
def add_noise_ceiling(ax, nc: pd.DataFrame, model_set: str) -> None:
    ...
    for vals, color, linestyle in [
        ...
        err = sem(vals)
        ax.axhspan(mean - err, mean + err, color=color, alpha=NC_BAND_ALPHA, zorder=0, linewidth=0)
        ax.axhline(
            mean,
            color=color,
            linewidth=0.85,
...
def draw_box(ax, x: float, vals: np.ndarray, *, color: str, width: float = 0.30) -> None:
...
def panel_data(df: pd.DataFrame, model_set: str) -> pd.DataFrame:
    models = set(constants.MODEL_SETS[model_set])
...
def draw_delta_summary(ax, panel: pd.DataFrame, c_col: str, v_col: str, xpos: float) -> None:
    ...
    ax.hlines([mean - err, mean + err], xpos - 0.07, xpos + 0.07, colors="0.12", linewidth=0.75, zorder=7)
...
def plot_panel(
    ...
    *,
    ...
    if show_xticks:
        ...
        labels = [
            SHORT_MODEL_NAMES.get(m, constants.MODEL_DISPLAY_NAMES.get(m, m)) for m in order
        ] + [r"$\Delta$", "range"]
    ...
    if show_legend:
        handles = [
            Line2D([0], [0], marker="o", color="none", markerfacecolor=COLOR_CSTIM, markeredgecolor=COLOR_CSTIM, markersize=4, label="Controversial"),
            Line2D([0], [0], marker="o", color="none", markerfacecolor=COLOR_BASELINE, markeredgecolor=COLOR_BASELINE, markersize=4, label="Baseline"),
        ]
        ax.legend(
            handles=handles,
            loc="lower left",
...
def write_summary(df: pd.DataFrame, weight: float) -> None:
    ...
    out = FIGURE_DATA_DIR / f"brain_alignment_grid_w{weight:g}_summary.csv"
...
def main() -> None:
    ...
    ratios = [
        len(panel_data(df, model_set)["model"].unique())
        + (2.8 if model_set == "all_models" else 1.9)
        for model_set in PANEL_ORDER
    ]
    ylims = {
    ...
    fig = plt.figure(figsize=(W_DOUBLE, 13.2))
    gs = fig.add_gridspec(
        len(ROW_SPECS),
        len(PANEL_ORDER),
        width_ratios=ratios,
    ...
    stem = f"target_adaptation_fixed_weight_brain_alignment_grid_w{args.weight:g}_cached"
