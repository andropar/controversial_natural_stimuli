...
FIGURE_DATA_DIR = REFIT_RESULTS_DIR / "figure_data"
...
POINTS_CSV = FIGURE_DATA_DIR / "weight0_to_best_cstim_points.csv"
SUMMARY_CSV = FIGURE_DATA_DIR / "weight0_to_best_cstim_summary.csv"
...
def short_model_label(model: str) -> str:
    return SHORT_MODEL_NAMES.get(model, constants.MODEL_DISPLAY_NAMES.get(model, model))
...
def plot_delta_grid(points: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    pdf = OUTPUT_FIGURES_DIR / f"{FIGURE_STEM}.pdf"
    png = OUTPUT_PNG_DIR / f"{FIGURE_STEM}.png"
    fig.savefig(pdf)
