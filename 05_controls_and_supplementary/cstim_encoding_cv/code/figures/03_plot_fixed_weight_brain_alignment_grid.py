...
def plot_panel(
    ...
    *,
    ...
    if show_xticks:
        ...
        labels = [
            SHORT_MODEL_NAMES.get(m, config.MODEL_DISPLAY_NAMES.get(m, m)) for m in order
        ] + [r"$\Delta$", "range"]
...
def write_summary(df: pd.DataFrame, weight: float) -> None:
    ...
    out = RESULTS_DIR / f"target_adaptation_fixed_weight_brain_alignment_grid_w{weight:g}_summary.csv"
