# Figures

<!-- BEGIN AUTO-FIGURE-PROVENANCE -->
## Contents Snapshot

- Folder: `05_controls_and_supplementary/simulation_validation/code/figures`
- Figures in this folder tree: 0
- Data/table-like files in this folder tree: 0
- Python scripts in this folder tree: 1
- Direct files: `plot_simulation_vs_fmri.py`

Use the tables below as a trace from rendered files back to the nearby code, staged data, score tables, or reports that produced them.

## Plotting Scripts

These scripts are kept here because they generate or document figure outputs elsewhere in the staged package.

| Script | Derived figure outputs | Resources or results referenced by script |
|---|---|---|
| `05_controls_and_supplementary/simulation_validation/code/figures/plot_simulation_vs_fmri.py` | `06_manuscript/figures/04_dissociation/simulation_vs_fmri`; `06_manuscript/figures/archive/simulation_vs_fmri` | `option_b_pairwise.csv`; `prediction_summary.csv`; `DATA_DIR`; `SIM_DATA_DIR`; `FIG_DIR` |
<!-- END AUTO-FIGURE-PROVENANCE -->
