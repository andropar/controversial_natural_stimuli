...
from config import MODEL_DISPLAY_NAMES, MODEL_SETS
...
def best_frac(curve: pd.DataFrame) -> float:
    if curve.empty:
        return np.nan
    return float(curve.loc[curve["mrsa"].idxmax(), "layer_frac"])
...
def plot_for_set(summary: pd.DataFrame, cstim_set: str) -> None:
    ...
    for ax, model in zip(axes, model_order):
        ...
        ax.plot(shared["layer_frac"], shared["mrsa"], color=LINE_COLORS["shared"],
                linewidth=1.0, label="Shared")
        ax.plot(vicco["layer_frac"], vicco["mrsa"], color=LINE_COLORS["vicco"],
                linewidth=1.0, label="Vicco")
        ax.plot(cstim["layer_frac"], cstim["mrsa"], color=LINE_COLORS["cstim"],
        ...
        shared_frac = best_frac(shared)
        cstim_frac = best_frac(cstim)
