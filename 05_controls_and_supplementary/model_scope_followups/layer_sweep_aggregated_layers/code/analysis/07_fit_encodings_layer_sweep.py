...
from layers_config import (
    LAYER_SET_CHOICES,
    MODEL_LAYERS,
    MODEL_SOURCE,
    PAPER_AGG_FEATURE_PROTOCOL,
    STIMULUS_SETS,
    get_layer_set,
    layer_set_uses_srp,
)
    ...
    for start in tqdm(range(0, len(items), batch_size), desc="    extract", leave=False):
        ...
        for name in layer_names:
            if apply_srp:
                ...
                reduced = np.asarray(raw[name], dtype=np.float32)
                if reduced.ndim != 2:
                ...
                meta = {
                    ...
                    "srp_target_dim": 0,
                    "srp_seed": -1,
                    "original_feature_dim": int(reduced.shape[1]),
                    "stored_feature_dim": int(reduced.shape[1]),
                    "feature_protocol": PAPER_AGG_FEATURE_PROTOCOL,
                }
...
def fit_stream_layer(layer_name, features, responses, hlvis_mask, args):
    ...
    apply_srp = bool(getattr(args, "apply_srp", True))
    feature_dim = int(features.shape[1])
    enc["feature_protocol"] = np.array(
        FEATURE_PROTOCOL if apply_srp else PAPER_AGG_FEATURE_PROTOCOL
    )
    enc["srp_applied"] = np.array(apply_srp, dtype=bool)
    enc["srp_target_dim"] = np.array(SRP_TARGET_DIM if apply_srp else 0, dtype=np.int32)
    enc["srp_seed"] = np.array(SRP_SEED if apply_srp else -1, dtype=np.int32)
    enc["original_feature_dim"] = np.array(feature_dim, dtype=np.int32)
    enc["stored_feature_dim"] = np.array(feature_dim, dtype=np.int32)
...
def stream_main(args):
    ...
    args.apply_srp = layer_set_uses_srp(args.layer_set)
    ...
    print(
        f"[stream] layer_set={args.layer_set}, "
        f"feature_protocol={FEATURE_PROTOCOL if args.apply_srp else PAPER_AGG_FEATURE_PROTOCOL}",
        flush=True,
    )
...
def main():
    ...
    parser.add_argument("--layer-set", choices=LAYER_SET_CHOICES, default="configured",
    ...
    parser.add_argument("--mode", choices=["all", "features", "fit", "stream", "merge-stream"],
                        ...
                        help="'features' only prepares DeepVision feature caches; "
                             ...
                             "'stream' extracts layer chunks, fits, scores, "
