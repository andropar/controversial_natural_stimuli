...
LAYER_SETS: Dict[str, Dict[str, List[LayerSpec]]] = {
    ...
    # Same dense layer inventory, but keep each layer's paper-style aggregation:
    # GAP for spatial maps, CLS/mean-patch for token outputs, and squeeze for
    # already-vector outputs. This intentionally avoids flatten+SRP.
    "dense_paperagg": DENSE_MODEL_LAYERS,
}

LAYER_SET_CHOICES = tuple(LAYER_SETS)
LAYER_SETS_USING_SRP = {"dense"}
PAPER_AGG_FEATURE_PROTOCOL = "paperagg_actualdim_v1"


def layer_set_uses_srp(name: str) -> bool:
    return name in LAYER_SETS_USING_SRP
