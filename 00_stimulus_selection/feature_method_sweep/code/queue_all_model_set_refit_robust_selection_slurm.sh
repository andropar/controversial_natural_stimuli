...
NOISE_CACHE_ROOT="${NOISE_CACHE_ROOT:-/ptmp/rothj/cstims_refit_robust_noise_cache}"
...
ALPHA_TARGET_BATCH_SIZE="${ALPHA_TARGET_BATCH_SIZE:-4096}"
SCORE_TARGET_BATCH_SIZE="${SCORE_TARGET_BATCH_SIZE:-8192}"
REFIT_SCORE_WORKERS="${REFIT_SCORE_WORKERS:-1}"
...
build_script_cmd() {
  ...
  local effective_noise_cache_dir="${NOISE_CACHE_DIR:-${NOISE_CACHE_ROOT}/${RUN_STAMP}/${model_set}}"
  SCRIPT_CMD=(
    ...
    --alpha-target-batch-size "${ALPHA_TARGET_BATCH_SIZE}"
    --score-target-batch-size "${SCORE_TARGET_BATCH_SIZE}"
    --noise-cache-dir "${effective_noise_cache_dir}"
...
EOF
  ...
  for name in \
    ...
    REFIT_OBJECTIVE KERNEL_BATCH_SIZE ALPHA_TARGET_BATCH_SIZE SCORE_TARGET_BATCH_SIZE \
    REFIT_SCORE_WORKERS PRECOMPUTE_BASE_KERNELS \
