#!/usr/bin/env python3
"""Plot final-stimuli teacher/student recovery curves."""
...
def plot_summary(summary: pd.DataFrame, out_base: Path) -> None:
    ...
    for r, report_track in enumerate(report_tracks):
        for c, model_set in enumerate(model_sets):
            ...
            for subset_type, ls in [("random", "--"), ("selected", "-")]:
                ...
                if subset_type == "random" and "recovery_accuracy_sd" in g:
                    band = g["recovery_accuracy_sd"].astype(float).fillna(0.0).to_numpy()
                    band_alpha = 0.22
                elif "recovery_accuracy_sem" in g:
                    band = (
                        CI_MULT
    ...
    if handles:
        ...
        labels.extend(["Random +/- SD", "Selected 95% CI"])
        fig.legend(handles, labels, loc="upper center", ncol=2, frameon=False, bbox_to_anchor=(0.5, 1.02))
    fig.suptitle("Teacher/student recovery: final selected stimuli", y=1.06)
    fig.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument("--name", default="teacher_student_recovery_final_stimuli_ns20")
    ...
    copy_png(args.figures_root, args.name)
