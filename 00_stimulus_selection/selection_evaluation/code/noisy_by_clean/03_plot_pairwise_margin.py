...
DEFAULT_FIGURES = EVAL_ROOT / "final_stimuli_recovery" / "noisy_by_clean" / "figures"
DATA_SUFFIX = "_noisy_by_clean_boot"
OUTPUT_PREFIX = ""
...
def load_pairwise(results_root: Path, model_set: str) -> pd.DataFrame:
    path = results_root / f"{model_set}{DATA_SUFFIX}" / "pairwise_margin.csv"
    if not path.exists():
        return pd.DataFrame()
    df = pd.read_csv(path)
...
def plot_track_group(results_root: Path, figures_root: Path, name: str, tracks: list[str]) -> None:
    ...
    pdf_path = figures_root / f"{OUTPUT_PREFIX}pairwise_dominance_margin_{name}.pdf"
    png_path = png_dir / f"{OUTPUT_PREFIX}pairwise_dominance_margin_{name}.png"
...
def main() -> None:
    global DATA_SUFFIX, OUTPUT_PREFIX
    args = parse_args()
